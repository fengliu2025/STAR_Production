...
int StPicoLambdaAnaMaker::InitHF() {
  ...
  if(isMakerMode() == StPicoHFMaker::kAnalyze)
  {
    ...
    ntp_Lambda->Branch("mNTrigs", &mNTrigs, "mNTrigs/I");
    ntp_Lambda->Branch("mTrigId", mTrigId, "mTrigId[mNTrigs]/I");

    //high pt particles
    ntp_Lambda->Branch("mNTrks", &mNTrks, "mNTrks/I");
    ...
    ntp_Lambda->Branch("p2_ch", p2_ch, "p2_ch[NLambda]/F");               //Float_t p2_ch
    ...
    //add track information here, Feng Liu 2026/7/21
    ntp_Lambda->Branch("track_Number"   ,&track_Number     ,"track_Number/I");
    ntp_Lambda->Branch("track_pt"       ,track_pt         ,"track_pt[track_Number]/F");
    ntp_Lambda->Branch("track_eta"      ,track_eta        ,"track_eta[track_Number]/F");
    ...
    */
    ...
    ntp_K0s->Branch("mNTrigs", &mNTrigs, "mNTrigs/I");
    ntp_K0s->Branch("mTrigId", mTrigId, "mTrigId[mNTrigs]/I");



    //high pt particles
    ntp_K0s->Branch("mNTrks", &mNTrks, "mNTrks/I");
    ntp_K0s->Branch("high_pt", high_pt, "high_pt[mNTrks]/F");  
    ntp_K0s->Branch("high_phi", high_phi, "high_phi[mNTrks]/F");
    ntp_K0s->Branch("high_eta", high_eta, "high_eta[mNTrks]/F");
    ...
    ntp_K0s->Branch("NK0s", &NK0s, "NK0s/I");
    //Pion 1
    ntp_K0s->Branch("p1_InEventID", &p1_InEventID, "p1_InEventID[NK0s]/I");               //Float_t p1_InEventID
    ntp_K0s->Branch("p1_pt", &p1_pt, "p1_pt[NK0s]/F");               //Float_t p1_pt
    ntp_K0s->Branch("p1_phi", &p1_phi, "p1_phi[NK0s]/F");             //Float_t p1_phi
    ntp_K0s->Branch("p1_eta", &p1_eta, "p1_eta[NK0s]/F");             //Float_t p1_eta
    ntp_K0s->Branch("p1_dca", &p1_dca, "p1_dca[NK0s]/F");               //Float_t p1_dca
    ntp_K0s->Branch("p1_ch", &p1_ch, "p1_ch[NK0s]/I");               //Int_t p1_ch
    ntp_K0s->Branch("p1_hasTOFinfo", &p1_hasTOFinfo, "p1_hasTOFinfo[NK0s]/I");   //Float_t p1_hasTOFinfo
    ntp_K0s->Branch("p1_dedx", p1_dedx, "p1_dedx[NK0s]/F");
    ntp_K0s->Branch("p1_beta", p1_beta, "p1_beta[NK0s]/F");
    ntp_K0s->Branch("p1_dcaxy",p1_dcaxy,"p1_dcaxy[NK0s]/F"); //Feng Liu 2026 7/24
    ntp_K0s->Branch("p1_nSigmaPion",p1_nSigmaPion,"p1_nSigmaPion[NK0s]/F");//Feng Liu 2026 7/24



    //pion 2
    ntp_K0s->Branch("p2_InEventID", &p2_InEventID, "p2_InEventID[NK0s]/I");               //Float_t p1_InEventID
    ntp_K0s->Branch("p2_pt", &p2_pt, "p2_pt[NK0s]/F");               //Float_t p2_pt
    ntp_K0s->Branch("p2_phi", &p2_phi, "p2_phi[NK0s]/F");             //Float_t p2_phi
    ntp_K0s->Branch("p2_eta", &p2_eta, "p2_eta[NK0s]/F");             //Float_t p2_eta
    ntp_K0s->Branch("p2_dca", &p2_dca, "p2_dca[NK0s]/F");               //Float_t p2_dca
    ntp_K0s->Branch("p2_ch", &p2_ch, "p2_ch[NK0s]/I");               //Float_t p2_ch
    ntp_K0s->Branch("p2_hasTOFinfo", &p2_hasTOFinfo, "p2_hasTOFinfo[NK0s]/I");   //Float_t p2_hasTOFinfo
    ntp_K0s->Branch("p2_dedx", p2_dedx, "p2_dedx[NK0s]/F");
    ntp_K0s->Branch("p2_beta", p2_beta, "p2_beta[NK0s]/F");
    ntp_K0s->Branch("p2_dcaxy",p2_dcaxy,"p2_dcaxy[NK0s]/F");        //Feng Liu 2026 7/24
    ntp_K0s->Branch("p2_nSigmaPion",p2_nSigmaPion,"p2_nSigmaPion[NK0s]/F"); //Feng Liu 2026 7/24



    //pair
    ntp_K0s->Branch("pair_charge", &pair_charge, "pair_charge[NK0s]/I");      //Int_t charge
    ntp_K0s->Branch("pair_DCAdaughters", &pair_DCAdaughters, "pair_DCAdaughters[NK0s]/F");      //Int_t pair_DCAdaughters
    ntp_K0s->Branch("pair_theta", &pair_theta, "pair_theta[NK0s]/F");   //Float_t pair_theta
    //ntp_K0s->Branch("pair_cosThetaStar", &pair_cosThetaStar, "pair_cosThetaStar/F");   //Float_t pair_cosThetaStar
    ntp_K0s->Branch("pair_decayL", &pair_decayL, "pair_decayL[NK0s]/F");  //Float_t pair_decayL
    ntp_K0s->Branch("pair_phi", &pair_phi, "pair_phi[NK0s]/F");      //Float_t pair_phi
    ntp_K0s->Branch("pair_eta", &pair_eta, "pair_eta[NK0s]/F");      //Float_t pair_eta
    ntp_K0s->Branch("pair_pt", &pair_pt, "pair_pt[NK0s]/F");        //Float_t pair_pt
    ntp_K0s->Branch("pair_mass", &pair_mass, "pair_mass[NK0s]/F");    //Float_t pair_mass
    ...
    ntp_K0s->Branch("track_Number"   ,&track_Number     ,"track_Number/I");
    ntp_K0s->Branch("track_pt"       ,track_pt         ,"track_pt[track_Number]/F");
    ntp_K0s->Branch("track_eta"      ,track_eta        ,"track_eta[track_Number]/F");
    ntp_K0s->Branch("track_phi"      ,track_phi        ,"track_phi[track_Number]/F");
    ntp_K0s->Branch("track_InEventID",track_InEventID  ,"track_InEventID[track_Number]/I");
    ntp_K0s->Branch("track_dca"      ,track_dca        ,"track_dca[track_Number]/F");
    ntp_K0s->Branch("track_dcaxy"      ,track_dcaxy        ,"track_dcaxy[track_Number]/F");
    ntp_K0s->Branch("track_nSigmaPion"   ,track_nSigmaPion     ,"track_nSigmaPion[track_Number]/F");
    ntp_K0s->Branch("track_nSigmaKaon"   ,track_nSigmaKaon     ,"track_nSigmaKaon[track_Number]/F");
    ntp_K0s->Branch("track_nSigmaProton" ,track_nSigmaProton   ,"track_nSigmaProton[track_Number]/F");
    ntp_K0s->Branch("track_dedx"         ,track_dedx           ,"track_dedx[track_Number]/F");
    ntp_K0s->Branch("track_ch"           ,track_ch             ,"track_ch[track_Number]/I");
...
int StPicoLambdaAnaMaker::createCandidates() {
  ...
  for (unsigned short idxPion = 0; idxPion < mIdxPicoPions.size(); ++idxPion)
    {
      StPicoTrack const *pion = mPicoDst->track(mIdxPicoPions[idxPion]);
      float eta = pion->gMom().PseudoRapidity();;
      heta_pi->Fill(eta);
    }
  ...
  for (unsigned short idxProton = 0; idxProton < mIdxPicoProtons.size(); ++idxProton)
    {
      StPicoTrack const *proton = mPicoDst->track(mIdxPicoProtons[idxProton]);
      float eta = proton->gMom().PseudoRapidity();
      heta_p->Fill(eta);
    }
  ...
  for (unsigned short idxPion = 0; idxPion < mIdxPicoPions.size(); ++idxPion)
  {
    ...
    */
    ...
    for (unsigned short idxPion2 = idxPion+1; idxPion2 < mIdxPicoPions.size(); ++idxPion2)
    {
      ...
      StHFPair pair2(pion, pion2, mHFCuts->getHypotheticalMass(StHFCuts::kPion), mHFCuts->getHypotheticalMass(StHFCuts::kPion), mIdxPicoPions[idxPion], mIdxPicoPions[idxPion2], mPrimVtx, mBField, false);
...
int StPicoLambdaAnaMaker::analyzeCandidates() {
  ...
  for(Int_t i=0;i<nNodes;i++){
    ...
    float pt = pTrack->pMom().Pt();
    float eta = pTrack->pMom().Eta();
    float phi = pTrack->pMom().Phi();
  ...
  //cout<<"middle 1 analyzeCandidates"<<endl;
  
  if(mIdSubLeadingPart != -1 )
  {
  ...
  if( mPicoHFEvent->nHFSecondaryVertices() > 0 )
  {
    for (unsigned int idx = 0; idx <  mPicoHFEvent->nHFSecondaryVertices(); ++idx)
    {
      ...
      //if(pair->m()<0.8) continue; // for lambda 
      if(pair->m()>0.8) continue; // for kaon
      ...
      //get beta of track from TOF, if TOF information is available          
      p1_hasTOFinfo[NK0s] = 0;

      if(!isnan(part1BetaBase) && part1BetaBase > 0)
      {
        p1_hasTOFinfo[NK0s] = 1;      
      }
      ...
      //get beta of track from TOF, if TOF information is available          
      p2_hasTOFinfo[NK0s] = 0;

      if(!isnan(part2BetaBase) && part2BetaBase > 0)
      {
        p2_hasTOFinfo[NK0s] = 1;
      }
      ...
      p1_InEventID[NK0s] = pair->particle1Idx();
      p1_pt[NK0s]  = part1->gPt();
      p1_phi[NK0s] = part1->gMom(pair->decayVertex(), mBField).Phi(); //20240215 update - calculate at SV, check usage of B field
      ...
      Int_t bTofPidTraitsIndex1          = part1->bTofPidTraitsIndex();
      ...
      p2_InEventID[NK0s] = pair->particle2Idx();
      p2_pt[NK0s] = part2->gPt();
      p2_phi[NK0s] = part2->gMom(pair->decayVertex(), mBField).Phi(); //20240215 update - calculate at SV, check usage of B field
      p2_eta[NK0s] = part2->gMom(pair->decayVertex() ,mBField).PseudoRapidity(); 
      p2_dcaxy[NK0s] = part2->gDCAxy(mPrimVtx.x(), mPrimVtx.y());
      p2_dca[NK0s] = part2->gDCA(vtxPos).Mag(); //Feng Liu 2026/7/24
      p2_nSigmaPion[NK0s] = part2->nSigmaPion();
      p2_ch[NK0s] = part2->charge();
      p2_dedx[NK0s] = part2->dEdx();
      p2_beta[NK0s] =-999.0;

      Int_t bTofPidTraitsIndex2          = part2->bTofPidTraitsIndex();
      ...
      pair_charge[NK0s] = -99;
      if( (part1->charge() + part2->charge()) == 0 ) pair_charge[NK0s] = 0; // unlike-sign - from decay of Lambda or K0s
      else if( fabs(part1->charge() + part2->charge()) == 2 ) pair_charge[NK0s] = 1; // unlike-sign - combinatorial background

      pair_DCAdaughters[NK0s] = pair->dcaDaughters();

      thetaProdPlane[NK0s] = pair->thetaProdPlane();

      prodPlane_x[NK0s] = pair->prodPlane().x();
      prodPlane_y[NK0s] = pair->prodPlane().y();
      prodPlane_z[NK0s] = pair->prodPlane().z();

      //cout<<pair->thetaProdPlane()<<endl;

      pair_theta[NK0s] = pair->pointingAngle();
      pair_cosThetaStar[NK0s] = pair->cosThetaStar();
      pair_decayL[NK0s] = pair->decayLength();
      pair_phi[NK0s] = pair->phi();
      pair_eta[NK0s] = pair->eta();
      pair_pt[NK0s] = pair->pt();
      pair_mass[NK0s] = pair->m();
  ...
  // add track information here, Feng Liu 2026/7/21
  track_Number = 0;
  // only fill the track information when there are at least two lambda candidates. 
  Int_t nTrk = mPicoDst->numberOfTracks();
  TVector3 PrimaryVtxPos          = picoEvent->primaryVertex(); 
  ...
  if(NK0s > 1){
        for(Int_t i=0;i<nTrk;i++){
                ...
                track_pt[track_Number]          = trk->gPt();
                track_eta[track_Number]         = trk->gMom().PseudoRapidity();
