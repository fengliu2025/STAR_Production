...
int StPicoLambdaAnaMaker::analyzeCandidates() {
  ...
  if( mPicoHFEvent->nHFSecondaryVertices() > 0 )
  {
    for (unsigned int idx = 0; idx <  mPicoHFEvent->nHFSecondaryVertices(); ++idx)
    {
      ...
      thetaProdPlane[NLambda] = pair->thetaProdPlane();

      prodPlane_x[NLambda] = pair->prodPlane().x();
      prodPlane_y[NLambda] = pair->prodPlane().y();
      prodPlane_z[NLambda] = pair->prodPlane().z();
